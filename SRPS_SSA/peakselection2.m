...
%
function [guessf] = peakselection2(niter,peakf,peakA,f,A, preBPM,preavgBPM,maA,realbpm,MAf,isplot,peakh)
...
fcost = ones(numchannel,20) * 999;
Acost = ones(numchannel,numchannel,20) * 999;
cost = ones(numchannel,20) * 999;
for c = 1:numchannel
    if ~isempty(fcandidate(c,:))
        for j = 1:length(fcandidate(c,:))
            ...
            for k = 1:numchannel
                Acost(c,k,j) = costA(peakA(c,heartindex(c,j)),magrad,costAtype);
            end;
...
if magrad > 0.0015 % increasing
    preBPM = preBPM + BPMdelta;
    preavgBPM = preavgBPM + BPMdelta;
