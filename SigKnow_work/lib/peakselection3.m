...
%
function [guessf] = peakselection3(niter,peakf,peakA,f,A, preBPM,preavgBPM,maA,realbpm,MAf,MAp,isplot,peakh)
...
if selecttype == 1
    guessf = costbase(niter,fcandidate, peakA, heartindex, preBPM, preavgBPM, maxABPM, magrad, costftype, costAtype, numchannel);
elseif selecttype == 2
    guessf = rulebase(niter,fcandidate, peakA, heartindex, preBPM, preavgBPM, maxABPM, MAf, magrad, costftype, costAtype, numchannel);
end;
...
function [guessf] = rulebase(niter,fcandidate, peakA, heartindex, preBPM, preavgBPM, maxABPM, MAf, magrad, costftype, costAtype, numchannel)
guessf = zeros(numchannel,1);
...
function [guessf] = costbase(niter,fcandidate, peakA, heartindex, preBPM, preavgBPM, maxABPM, magrad, costftype, costAtype, numchannel)
guessf = zeros(numchannel,1);
...
%             cost(c,j) = fcost(c,j) + 2*((Acost(c,c,j))+sum(Acost(c,:,j)))/(numchannel+1);
            cost(c,j) = fcost(c,j) + 10*(Acost(c,c,j));
        end;
        
        % =================================================
%         mincost = min(cost(c,:));
%         finalcandidate = fcandidate(c,find(cost(c,:) > 0.8*mincost));
%         dis = 100;
%         guessf(c,1) = finalcandidate(1);
%         for fi = 1:length(finalcandidate)
%             if abs(finalcandidate(fi) - preBPM/60) < dis
%                 dis = abs(finalcandidate(fi) - preBPM/60);
%                 guessf(c,1) = finalcandidate(fi);
%             end
%         end
        
        % =================================================
 
        if length(find(cost(c,:) == min(cost(c,:)))) ~= 1
