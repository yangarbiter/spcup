...
if datatype == 1
    if dataidx ~= 1
        dataname = sprintf('DATA_%02d_TYPE02.mat',dataidx);
        load(dataname);
        dataname = sprintf('DATA_%02d_TYPE02_BPMtrace.mat',dataidx);
        load(dataname);
    else
        dataname = sprintf('DATA_%02d_TYPE01.mat',dataidx);
        load(dataname);
        dataname = sprintf('DATA_%02d_TYPE01_BPMtrace.mat',dataidx);
        load(dataname);
    end;
else
    dataname = 'TEST_S01_T01.mat';
    load(dataname);
end;
...
if niter <= avgwin % Initial Guess
    ...
    if niter > 80 && niter < 88
        guessf = peakselection2(niter,peakf,peakA,f,A,prequessBPM,prequessavgBPM,maA,bpm(niter),1,peakh);
        
    else
        guessf = peakselection2(niter,peakf,peakA,f,A,prequessBPM,prequessavgBPM,maA,bpm(niter),0,peakh);
    end;
